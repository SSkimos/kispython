...
def f_spring(u, v):  # guk
    return ((v - u) / (np.linalg.norm(v - u))) * C1 * math.log(np.linalg.norm(u - v) / C2)


def f_ball(u, v):  # kulon
    return ((u - v) / (np.linalg.norm(u - v))) * (C3 / (np.linalg.norm(u - v)) ** 2)


def force_layout(nodes):  # summary
    forces = {}
    for u in nodes:
        sum = np.array([0.0, 0.0])
        for v in all_nodes:
            listN = np.array([u.vec.x, u.vec.y])
            listI = np.array([v.vec.x, v.vec.y])
            if v is u:
                continue
            elif v in u.targets:
                sum += f_spring(listN, listI)
            else:
                sum += f_ball(listN, listI)
        forces[u] = sum
    for n in nodes:
        item = forces[n]
        n.vec.x += item[0] * C4
